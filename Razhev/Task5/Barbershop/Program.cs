...
namespace Barbershop
{
    class Program
    {
        ...
        private static int countSuccessfulVisitor = 0;
        private static bool verify = false; //  не занято

        private static object mSyncObj = new object();
        private static bool mStop = true;
        ...
        public static void VisitorExit(object obj)
        {
            if(countVisitor >= 1)
            {
                ...
                countSuccessfulVisitor++;
                if (countSuccessfulVisitor == 1)
                {
                    Console.WriteLine($"Парикмахер подстриг: {countSuccessfulVisitor} поситителя.");
                }
                else
                {
                    Console.WriteLine($"Парикмахер подстриг: {countSuccessfulVisitor} поситителей.");
                }
                if(countVisitor == 0)
                {
